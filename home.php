...
<main id="main-content" class="posts-archive container">
    ...
    <?php if (have_posts()) : ?>
        ...
        <nav class="posts-pagination" aria-label="Más publicaciones">
            <?php next_posts_link('Ver más publicaciones'); ?>
        </nav>
        <p class="posts-load-status" role="status" aria-live="polite"></p>
        <div class="posts-sentinel" aria-hidden="true"></div>
    <?php else : ?>
        <p class="posts-empty">Pronto compartiremos aquí nuevas publicaciones para tu negocio.</p>
    <?php endif; ?>
</main>
